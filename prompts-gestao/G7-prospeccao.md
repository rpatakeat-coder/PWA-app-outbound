...
**Arquivo:** `gestao/src/telas/Prospeccao.tsx` · **Dados:** `gestao/src/dados/prospeccao.ts`
**Referência visual:** o preview `Cockpit de Gestão - Casca web.dc.html`, aba **Prospecção**
**Pergunta que a tela responde:** *O que entra no topo do funil?* — é o subtítulo no header, e o critério para decidir o que fica e o que sai
...
Quatro títulos de seção no código: **"Do estoque à venda"** (L115), **"A régua de hoje"** (L148), **"Por praça"** (L196) e **"Descarte em campo · {n}"** (L240). É a tela mais estruturada das sete — a reforma é de grade e densidade, não de conteúdo.

## O que fazer

**Faixa de KPIs** em 4 colunas: estoque · entraram na semana · Conta Alvo · descartados.

**Grade `7fr 5fr`, gap 20, `align-items:start`** — quatro blocos, dois de cada lado:

**Do estoque à venda** (7fr) — funil de conversão, não de etapa: cada barra é um estágio de afunilamento ("Estoque disponível" → "Primeira visita feita" → "Chegou ao decisor" → "Demo realizada" → "Fechou"), largura proporcional ao **primeiro**, para a queda ser visível. Barra de 20px, raio 4, trilha `--sunk`.

**A régua de hoje** (5fr) — nota "quem cobrar". É a lista de ação: rótulo da regra à esquerda, contagem à direita em `tabular-nums`, badge de urgência — **Estourado** `--red-soft`/`--red` · **Vence hoje** `--amber-soft`/`--amber-ink` · **Reciclar** e **Escalar** com o tint que couber.

Os critérios vêm de `src/utils/sla.ts` e de `prospeccao.ts` — **não invente régua**.

**Por praça** (7fr) — barras, mesma anatomia, proporcional à maior praça.

**Descarte em campo · {n}** (5fr) — nota "motivo declarado". Lista simples: motivo à esquerda, contagem à direita. **Sem badge** — é diagnóstico, não ação.

> "Descarte em campo" é o feedback do vendedor voltando para a prospecção. Se "Endereço não existe" for grande, o problema está na origem da base, não no time. Vale manter visível mesmo quando pequeno.
