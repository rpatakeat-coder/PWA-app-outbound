...
//
// Quando o RPA manda o lead sem logradouro, o Nominatim costuma cair no
// centroide do CEP/bairro, empilhando varios clientes no mesmo pino. Por
// isso este webhook faz lookup do logradouro via ViaCEP/BrasilAPI quando
// o payload vier sem ele, e marca geo_approximate=true quando o hit do
// Nominatim parece ser centroide (postcode/suburb/road/etc.) — assim o
// cron de reparo pega esses casos no proximo ciclo.
//
// Deploy:
...
async function fetchJson(url: string): Promise<any | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[hubspot-lead-webhook] fetch non-ok', res.status, url);
      return null;
    }
    return await res.json();
  } catch (err) {
    console.warn('[hubspot-lead-webhook] fetch failed', url, err);
    return null;
  } finally {
    clearTimeout(timer);
  }
